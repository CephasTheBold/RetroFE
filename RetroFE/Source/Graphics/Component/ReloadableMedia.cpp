...
bool ReloadableMedia::update(float dt) {
	// 1. --- Force reload on playlist change for playlist-driven types ---
	...
	// 2. --- THE FIX: Selection Guard ---
	if (newItemSelected || (newScrollItemSelected && getMenuScrollReload())) {

		Item* current = page.getSelectedItem(displayOffset_);

		// If the physical item hasn't changed (e.g., returning from a game), 
		// clear the flags and skip the reload entirely. This prevents the 
		// redundant call to allocateGraphicsMemory() that causes the flash.
		if (current == lastItem_) {
			newItemSelected = false;
			newScrollItemSelected = false;
			if (loadedComponent_) {
				loadedComponent_->clearPendingRequests();
			}
		}
		else {
			lastItem_ = current;
			...
			if (foundComponent) {
				...
				// Only call this for a TRULY new item selection.
